...
export default function CompanyDashboardPage() {
  ...
  const fetchCompanyData = useCallback(async () => {
    ...
    try {
      ...
      const allBookings: Booking[] = Array.isArray(res)
        ? res
        : (res as { data?: Booking[] }).data || []
      if (allBookings.length > 0) {
        setBookingCount(allBookings.length)
        const active = allBookings.filter((b) =>
          (
            ['ACCEPTED', 'WAY_TO_PICKUP', 'ARRIVED', 'PICKED_UP', 'WAY_TO_DESTINATION'] as string[]
          ).includes(b.status)
        )
        ...
        setRecentBookingsData(
          sorted.map((b) => {
            const statusMap: Record<string, string> = {
              ...
              WAY_TO_PICKUP: 'In Progress',
              ARRIVED: 'In Progress',
              PICKED_UP: 'In Progress',
              WAY_TO_DESTINATION: 'In Progress',
            }
            return {
            ...
            <div className="relative h-full overflow-hidden rounded-2xl border border-white/[0.06] bg-gradient-to-br from-accent/[0.06] via-white/[0.02] to-transparent p-6">
              ...
              <div className="relative flex items-center justify-between">
                ...
                <svg width="120" height="50" className="hidden sm:block" viewBox="0 0 120 50">
                  ...
                  <path
                    d="M0,35 L20,30 L40,32 L60,20 L80,25 L100,12 L120,8"
                    fill="none"
                  ...
                  />
                  <path
                    d="M0,35 L20,30 L40,32 L60,20 L80,25 L100,12 L120,8 L120,50 L0,50Z"
            ...
            <div className="relative overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.02]">
              ...
              <div className="flex items-center justify-between px-6 py-5">
                ...
                <Link
                  ...
                  className="text-sm text-accent font-semibold hover:text-accent-light transition-colors flex items-center gap-1"
                >
              ...
              <div className="px-6 pb-6 space-y-3">
                {recentBookingsData.length === 0 ? (
                  ...
                  recentBookingsData.map((b, i) => (
                    <div
                      key={i}
                      className="group flex flex-col sm:flex-row sm:items-center gap-3 p-4 rounded-xl bg-white/[0.02] border border-white/[0.04] hover:bg-white/[0.04] hover:border-white/[0.08] transition-all duration-300"
                    >
                      {/* Employee */}
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        <span
                          className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${getStatusStyle(b.status)}`}
                        >
                          {b.status}
                        </span>
                      </div>
                ...
                {topEmployeesData.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {topEmployeesData.map((emp, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/[0.03] hover:bg-white/[0.04] transition-all"
                      >
                        {/* Rank */}
                        <div
