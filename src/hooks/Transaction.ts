...
import { ITransaction, IAdminTransaction, IWithdrawals, ICommission } from '@/types/type'
...
const getUserTransactions = async (id: string) => {
  const { data } = await axiosInstance.get(`/payment-transaction/transactions/${id}`)
  return data.data
}
...
export const useUsersTransactionsQuery = () => {
  return useQuery<ITransaction[], Error>({
    ...
    queryFn: async () => {
      const { data } = await axiosInstance.get(`/payment-transaction/admin/users`)
      return data.data
    },
  })
}
...
export const useWithdrawalRequestsQuery = () => {
  return useQuery<IWithdrawals[], Error>({
    ...
    queryFn: async () => {
      const withdrawals = await apiClient.get<IWithdrawals[]>(
        `/payment-transaction/admin/withdrawals`
      )
...
export const useCommissionsQuery = () => {
  return useQuery<ICommission, Error>({
    ...
    queryFn: async () => {
      const commissions = await apiClient.get<ICommission>(`/finance/commissions`)
