...
export default function PassengersQuickEditForm({
  currentUser,
  open,
  onClose,
  setChangeFlag,
  ...
  open: boolean;
  onClose: () => void;
  setChangeFlag: (value: boolean | ((prev: boolean) => boolean)) => void;
}) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      const response = await axiosInstance.put(
        `/admin/passengers/${currentUser?.id}`,
        formData
      );

      if (response.status === 200) {
        setChangeFlag((prev: boolean) => !prev);
